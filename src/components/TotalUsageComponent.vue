...
const formatTotalPower = computed(() => {
  return Number(powerUsage.value[0].total_power * (5 / 3600)).toFixed(2) + ' Wh'
})

const formatKWH = computed(() => {
  return Number((powerUsage.value[0].total_power * (5 / 3600)) / 1000).toFixed(2) + ' kWh'
})
        ...
        <div v-if="powerUsage" class="flex flex-row gap-5">
          ...
          <span class="">
            <button @click="togglePowerFormat" class="flex flex-row items-end group">
              ...
              <p class="text-sm text-gray-500 group-hover:text-black group-hover:font-semibold">
                Toggle mode
              </p>
            </button>
          </span>
